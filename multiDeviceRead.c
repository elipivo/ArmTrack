/*
 * Name: multiDeviceRead.c
 * Author: Elijah Pivo
 *
 * Description:
 * 	Tests timing architecture for multiple devices. (IMU, Force, CyGl, EMG)
 * 	This test uses a parallel background thread to print data.
 *
 * Usage:
 * 	Compile with: gcc -o multiDeviceRead multiDeviceRead.c quickDevice.c slowDevice.c -std=gnu99 -Wall -Wextra -pthread
...
pthread_t device4Thread;
pthread_mutex_t device4Lock;
pthread_cond_t device4Signal;
pthread_mutex_t device4CompleteLock;
pthread_cond_t device4CompleteSignal;
...
//	}
	...
	if (startSlowDevice(&data.slowDevice) != 1) {
		...
		pthread_mutex_init(&device4Lock, NULL);
		pthread_cond_init(&device4Signal, NULL);
		pthread_mutex_init(&device4CompleteLock, NULL);
		pthread_cond_init(&device4CompleteSignal, NULL);

		if (pthread_create(&device4Thread, NULL, getData4, NULL) != 0) {
			fprintf(stderr, "Couldn't start slowDevice data collection thread...\n");
			exit(1);
		}
	}
...
void getData() {
	...
	data.device4 = 1; //start slowDevice
...
void* getData4() {
	...
	while (1 == 1) {
		...
		if (data.device4 != 0) {
			//collect data
			getSlowDeviceData(&data.slowDevice, data.time);
			data.device4 = 2;
		}
...
void endSession() {
	...
	pthread_cancel(device4Thread);

	pthread_mutex_destroy(&printLock);
	pthread_mutex_destroy(&device1Lock);
	pthread_mutex_destroy(&device2Lock);
	pthread_mutex_destroy(&device3Lock);
	pthread_mutex_destroy(&device4Lock);
	pthread_cond_destroy(&printSignal);
	pthread_cond_destroy(&device1Signal);
	pthread_cond_destroy(&device2Signal);
	pthread_cond_destroy(&device3Signal);
	pthread_cond_destroy(&device4Signal);

	double percentMissed = (data.errors / (double) data.reads) * 100;

	fprintf(stderr, "Elapsed Time (sec): %05.3f\tNumber of Reads: %i\tPercent Missed: %5.3f%%\n",
			data.time, data.reads, percentMissed);
